...
class TransferirPlay {
  ...
  @given(
    "conta web {string} com saldo {float} e a conta web {string} com saldo {float}",
    { timeout: -1 }
  )
  public async dadaDuasContas(
    numeroOrigem: string,
    saldoOrigem: number,
  ...
  public async quandoTransferirValores(
    numeroOrigem: string,
    valor: number,
    numeroDestino: string
  ) {
  ...
  @then(
    "o saldo da conta web {string} deve ser {float} e a conta web {string} {float}"
  )
  public async entaoSaldoDeveSer(
    ...
    await expect(
      this.page.getByText(`Saldo:R$ ${resultadoOrigem.toFixed(2)}`)
    ).toBeVisible();
    await this.page.getByRole("link", { name: "Retornar" }).click();
    await this.page.getByTestId(`${numeroDestino}-ver`).click();
    await expect(
      this.page.getByText(`Saldo:R$ ${resultadoDestino.toFixed(2)}`)
    ).toBeVisible();
